 ...
 *  HTTPS GET sends the current CYBSP_LED2 status as a response to the client.
 *  HTTPS POST toggles the CYBSP_LED2 and then sends its current state as a
                                 ...
                                 cy_http_message_body_t* https_message_body)
{
    ...
    switch (https_message_body->request_type)
    {
        case CY_HTTP_REQUEST_GET:
             ...
             */
            if (CYBSP_LED_STATE_ON == cyhal_gpio_read(CYBSP_LED2))
            {
